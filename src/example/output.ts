...
export const PlayerState = {
	...
	validate(obj: PlayerState) {
		...
		validationErrors = validateOptional(obj.turn, (x) =>
			validatePrimitive(typeof x === 'string', `Invalid string: ${x}`)
		)
		if (validationErrors.length > 0) {
		...
		validationErrors = validateOptional(obj.winner, (x) =>
			validatePrimitive(typeof x === 'string', `Invalid string: ${x}`)
		)
		if (validationErrors.length > 0) {
		...
		validationErrors = validateOptional(obj.intOptional, (x) =>
			validatePrimitive(Number.isInteger(x), `Invalid int: ${x}`)
		)
		if (validationErrors.length > 0) {
